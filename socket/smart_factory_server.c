/* base : KSH */
/* modified by JUB, KMJ */
/* This file contains Smart Factory system's server */
/* 서울기술 교육센터 IoT */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
*/
    ...
    fputs("Smart Factory Server Start!!\n",stdout);
...
void  getlocaltime(char * buf)
{
    ...
    if(errno == EFAULT)
        perror("time()");
    t = localtime(&tt);
    sprintf(buf,"[GETTIME]%02d.%02d.%02d %02d:%02d:%02d %s",t->tm_year+1900-2000,
            t->tm_mon+1,t->tm_mday,t->tm_hour,t->tm_min,t->tm_sec,wday[t->tm_wday]);
